...
---------------------------------
This module implements least squares fit with numpy.
...
class LinregResult:
    ...
    def __repr__(self) -> str:
        return f"<LinregResult result={self.result!r} meta={self.meta}>"

def core_algorithm(data: Any, **kwargs: Any) -> LinregResult:
    """
    Core algorithm stub.
    Parameters
    ----------
    data : Any
        Input structure (least squares fit with numpy).
    kwargs : Any
    ...
    return LinregResult(result={"acc": acc, "trace": trace}, meta=meta)
...
def pretty_print(result: LinregResult) -> str:
    """Create a human-readable summary."""
    acc = result.result.get("acc")
    pts = result.meta.get("trace_points")
    return f"Linear Regression: acc={acc:.4f}, trace_points={pts}"
